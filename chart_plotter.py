...
class PatternDetector:
    ...
    # ... (Pattern detection methods remain unchanged, including detailed outputs)
...
def plot_chart(data, patterns=None, show_gann_levels=False, moving_averages=None):
    """Plot candlestick chart with customizable pattern filtering, Gann levels, and moving averages."""
    fig = go.Figure()
    ...
    for ticker, ticker_data in data.items():
        ...
        if moving_averages:
            for ma_period in moving_averages:
                ticker_data[f'MA_{ma_period}'] = ticker_data['Close'].rolling(window=ma_period).mean()
                fig.add_trace(go.Scatter(x=ticker_data.index, y=ticker_data[f'MA_{ma_period}'], 
...
class StockAnalyzerApp:
    ...
    def __init__(self, root):
        ...
        self.root.geometry("500x600")  # Increased height for additional controls
    ...
    def create_widgets(self):
        ...
        ttk.Label(self.root, text="Moving Averages (comma separated):", background="#f0f0f0").grid(column=0, row=13, padx=10, pady=10, sticky=tk.W)
        self.ma_entry = ttk.Entry(self.root, width=30)
        self.ma_entry.grid(column=1, row=13, padx=10, pady=10)

        # Analyze Button
        self.analyze_button = ttk.Button(self.root, text="Analyze", command=self.analyze_stock)
        self.analyze_button.grid(column=0, row=14, padx=10, pady=10)

        # Export Button
        self.export_button = ttk.Button(self.root, text="Export Results", command=self.export_results)
        self.export_button.grid(column=1, row=14, padx=10, pady=10)

        # Status Label
        self.status_label = ttk.Label(self.root, text="", background="#f0f0f0")
        self.status_label.grid(column=0, row=15, columnspan=2, padx=10, pady=10)

        # History Listbox
        self.history_listbox = tk.Listbox(self.root, width=50)
        self.history_listbox.grid(column=0, row=16, columnspan=2, padx=10, pady=10)
    ...
    def analyze_stock(self):
        ...
        moving_averages = [int(period) for period in self.ma_entry.get().strip().split(',') if period.isdigit()]
        ...
        def run_analysis():
            ...
            plot_chart(data, patterns, show_gann_levels, moving_averages)
